...
public class JdkResourcesServer {
	...
	/**
	 * @param jdkExecutable
	 * 		Path to JDK's {@code java} executable to initiate the server with.
	 *
	 * @return Server client for started process.
	 *
	 * @throws IOException
	 * 		If any I/O error occurs.
	 */
	public static JdkResourcesServer start(Path jdkExecutable) throws IOException {
		if (!Files.isRegularFile(jdkExecutable))
			throw new IllegalStateException("JDK executable does not exist: " + jdkExecutable);

		// Prepare jar
		Path classpathJar = JdkResourcesServer.classpathJar;
		if (classpathJar == null) {
			classpathJar = Files.createTempFile("cs-jdk-resources", ".jar");
